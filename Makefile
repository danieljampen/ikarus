...
avtest:
	@echo "===> Ikarus Version"
	@docker run --init --rm --entrypoint=bash $(ORG)/$(NAME):$(VERSION) -c "scan -v" > tests/av_version.out
	@echo "===> Ikarus VPS"
	@docker run --init --rm --entrypoint=bash $(ORG)/$(NAME):$(VERSION) -c "scan -V" > tests/av_vps.out
	@echo "===> Ikarus EICAR Test"
	@docker run --init --rm --entrypoint=bash $(ORG)/$(NAME):$(VERSION) -c "scan -abfu EICAR" > tests/av_scan.out || true
	@echo "===>  $(MALWARE) Test"
